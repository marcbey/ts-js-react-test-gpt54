...
const normalizeLocalizedText = (text: LocalizedText): LocalizedText => ({
...
const buildAnswer = (entry: DraftQuestion): LocalizedText => {
  const germanAnswer = refineGermanAnswer(entry.answer.de)
  const germanContext = firstSentence(refineGermanAnswer(interviewQuestionExplanationDetails[entry.id].de))
  const englishContext = firstSentence(interviewQuestionExplanationDetails[entry.id].en)
